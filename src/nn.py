...
def generate_data(N=1_000, n_loops=2.5, noise=0.6, seed=1234, debug=False):
...
def score(x, y, trainer, xlim, ylim):
    ...
    plt.title(
        f"acc={accuracy:.2f} | avg(prec)={average_precision:.2f} | rocauc={rocauc:.2f}"
    )
    plt.xlabel(r"$x_1$")
    plt.ylabel(r"$x_2$")
    plt.legend(loc="upper left")
